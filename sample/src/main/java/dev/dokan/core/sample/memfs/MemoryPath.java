...
public class MemoryPath implements Comparable<MemoryPath>, Iterable<MemoryPath> {
    ...
    public static final MemoryPath ROOT = new MemoryPath(new String[]{MAGIC_ROOT_STR});
    ...
    public static MemoryPath of(String path) {
        ...
        var components = path.trim().split(String.valueOf(SEPARATOR) + String.valueOf(SEPARATOR));
        if (path.indexOf(SEPARATOR) == 0) { //if path is empty, we must not change the first (and only) component!
    ...
    public MemoryPath toAbsolutePath() {
        if (isAbsolute()) {
            ...
            var absComponents = new String[components.length + 1];
            System.arraycopy(components, 0, absComponents, 1, components.length);
            return new MemoryPath(absComponents);
        }
    }
    ...
    public String toString() {
        if (this.equals(ROOT)) {
            ...
            return SEPARATOR + Arrays.stream(components).skip(1).collect(Collectors.joining(String.valueOf(SEPARATOR)));
        } else {
            return Arrays.stream(components).collect(Collectors.joining(String.valueOf(SEPARATOR)));
        }
    }
}
